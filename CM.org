...
#+BEGIN_THEOREM
L'enveloppe convexe est convexe : toute corde de l'enveloppe est comprise dans l'objet.

Les faux négatifs sont impossibles : Quand on dit que deux objets ne se touchent pas leurs enveloppes convexes à peu près, on sait qu'ils ne se touchent pas en vrai.

Les faux positifs sont aussi strictement moins fréquent qu'avec une enveloppe circulaire.
#+END_THEOREM

#+BEGIN_DEFINITION
On rappelle que le *sens trigonométrique* est le sens horaire pour les informaticiens : l'axe des ordonnées est dirigé vers le bas.
#+END_DEFINITION
...
On se propose de simplement itérer sur les points de p1 et p2 :
Prendre trois points de p1 (avec un itérateur bien pratique) (en faisant bien attention à ne pas passer past the end), et vérifier pour chaque point de p2 s'ils sont du même côté. S'ils ne le sont pas, on peut break, le côté considéré n'est pas un côté séparant.
...
En vrai, c'est beaucoup plus rapide : on fait plein de break et de return prematurés : à moins d'avoir vraiment très peu de chance, où c'est systématiquement le dernier point qu'on teste qui "casse" le côté.
...
Cet algorithme est en \Theta(nh)
