...
Chaque noeud a k+1 sous A_1, ..., A_{k+1} tels que :
...
Les noeuds internes contiennent des k-uplets de 1, 2 ou 3 éléments.

Toutes les feuilles sont au même niveau.
#+END_DEFINITION

#+BEGIN_THEOREM
h la hauteur d'un arbre 2-3-4 avec n éléments : h \in \Theta(n)
#+END_THEOREM

#+BEGIN_PROOF
Si un arbre n'a que des 2-noeuds (une seule valeur par ) :

h+1 = log_2(n+1)

Si un arbre n'a que des 4-noeuds (3 valeurs par ) :
...
On dispose d'un ensemble de primitives utiles, qui pourront être réutilisées en examen.
...
L'ajout est guidé par la recherche.

Je rajoute dans le seul noeud dans lequel je peux mettre (de plus bas niveau, bien sûr)

Si celui-ci est plein, alors on doit éclater le noeud (ce qui implique une modification de la valeur) :
...
On met l'élément à sa place en recommeçant à chercher au pointeur (qu'on a pris la peine de garder avant), dans l'endroit de plus bas niveau.

Le pointeur est un endroit d'où recommencer la recherche après l'éclatement.

Si on a un éclatement sur une feuille du bas, on doit faire remonter la valeur centrale de l'élément à éclater

[faire l'exemple avec les valeurs données]
...
En descente :
- Parcours uniquement de haut en bas (+)
- Transformation très locale (on éclate un noeud, et on remonte pas plus haut que le père) (+)
- Taux d'occupation plus faible (-)
- Hauteur plus grande (-)

En remontée :
- Parcours dans les deux sens (double pointeurs, empreinte mémoire) (-)
- Transformation en chaîne de proba non-nulle (-)
...
1 éclatement implique écrire deux pages en disque.
...
Quand je fais remonter qqch de très profond, l'arbre résultant est plutôt équilibré.
...
On a un ensemble de primitives utiles, qui vont permettre dé
...
Idéalement, on doit commencer par insérer du milieu d'alphabet (le truc le plus proche de :
10000000
soit la valeur à égale distance de 00000000 et de 11111111)
...
Notion de Patricia tries : on fusionne les noeuds qui n'ont qu'un fils avec le fils.
...
Par exemple, un 26-trie peut avoir tous les mots de la langue française en une structure unique (pourvu qu'on écrive pas les accents).
