...
Géométrie algorithmique
DirectX, OpenGL
Calculs dans les cartes graphiques

#+BEGIN_DEFINITION
Collision entre deux objets géométriques a lieu sssi l'intersection est non-nulle.
#+END_DEFINITION
...
Si un cercle de diamètre égal à la distance de deux points de la liste couvre tout autre point de la liste, alors ce cercle est un cercle couvrant de rayon minimum.
#+END_THEOREM

#+BEGIN_THEOREM
Lemme 2

En 2D, il existe un unique cercle passant par 3 points non-colinéaires.
#+END_THEOREM

#+BEGIN_THEOREM
Lemme 3 (personnel)

En 2D, il existe un unique cercle passant par les N sommets d'un polygone régulier. La preuve suit naturellement de la définition d'un polygone régulier.
#+END_THEOREM
...
#+BEGIN_THEOREM
Le problème du cercle minimum peut être résolu en temps O(n^4).
#+END_THEOREM

A-t-on un algorithme qui prouve ce théorème ?

Solution personnelle (je ne me rappelle plus de ce que le cours disait) :
On a 2 parmi n couple de points dans le nuage de n points, soit (n^2 - n) / 2.

On constitue ces couples et on calcule leur distance à la volée : O(7n^2)
On trie ces distances de la plus à la moins grande (on dit quicksort) : log(n^2)
On ne garde que les N strictement plus grandes que toutes les autres : O(cn)
...
de rayon = (d(C,S) + ancien rayon) / 2 (un calcul de distance déjà fait et un shift de 1 bit : 1 ALU)
et de centre en le point à rayon distance de S sur la droite CS (applique le vecteur, tya compris : 2 ALU, 1 pour les abscisses, 1 pour les ordonnées)
...
#+BEGIN_DEFINITION
"Le big data, c'est quand ça tient pas dans la RAM."

Binh-Minh Bui Xuan, 2019
#+END_DEFINITION
...
Les cercles crééent plein de faux positifs. On va essayer un meilleur conteneur, l'enveloppe convexe.

L'enveloppe convexe est convexe : toute corde est à l'intérieur de l'objet, ou alors, tous les points d'un segment formé par deux points dans l'espace convexe sont dans l'espace.

Permet de diminuer le taux de faux positifs, et on n'a pas de faux négatifs. Quand on dit que deux objets ne se touchent pas leurs enveloppes convexes à peu près, c'est toujours vrai.

Sens trigonométrique est le sens horaire pour les informaticiens. (y va vers le bas).
...
Complexité au pire cas : p1.size() * p2.size().
En vrai, c'est beaucoup plus rapide : on fait plein de break et de return prematurés

**** Aparté : 

Test de ce que qqch est à droite ou à gauche d'une droite :
On utilise le produit vectoriel : si la coordonnée z est positive, on est d'un côté, négative on est de l'autre. C'est comme ça que la fonction même côté est implémentée.
...
Complexité au pire cas est en n^3 (on se rend compte au dernier point que le segment qu'on regarde n'était en fait pas un côté)
En vrai, on est sur du n^2 : on voit très vite si on n'est pas un côté, et on break.

*** Meilleures solutions

**** Elimination par pixel + Graham
...
On créé deux séries de N buckets (N étant la largeur en pixels de votre écran).
...
**** Filtrage Akl-Toussaint + Quickhull

On saut que les 4 points extrêmaux (Nord, Sud, Est, Ouest) sont dans l'enveloppe convexe. Et on sait en plus que tous les points dans le quadrilatère ne peuvent pas être dans l'enveloppe convexe.
...
Complexité au pire cas n^2. \Theta(n^2)
...
On coupe la liste en morceaux, puis appels récursifs sur les .

Jarvis sur les contours positifs des morceaux.
...
[[./CM3/coursBMBX1.pdf][Cours 1]]
[[./CM4/coursBMBX2.pdf][Cours 2]]
